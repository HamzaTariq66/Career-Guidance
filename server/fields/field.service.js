  ...
  return new Promise((resolve, reject) => {
    let additional_where =
      typeof id !== "undefined" && id !== 0
        ? " where tbl1.degree_id = " + id
        : "";
    additional_where = additional_where
      ? additional_where
      : "group by tbl1.title";
    let sql =
      "SELECT tbl1.id, tbl1.title, tbl1.url, tbl1.degree_id,tbl2.title as degree FROM " +
      tablename +
      " as tbl1 join " +
      tablename2 +
      " as tbl2 on tbl2.id = tbl1.degree_id " +
      additional_where;
...
async function getById(id) {
  return new Promise((resolve, reject) => {
    let sql =
      "SELECT tbl1.*,tbl2.title as degree FROM " +
      tablename +
      " as tbl1 join " +
      tablename2 +
      " as tbl2 on tbl2.id = tbl1.degree_id where id = " +
      id;
    db.query(sql, (error, rows, fields) => {
      if (error) reject(error);
      else {
